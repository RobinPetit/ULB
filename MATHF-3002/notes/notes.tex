...
\[
	...
	= \abs z^{-1}\sum_{k=K_1}^{K_2}\left(\underbrace {\frac {\norm S}{\abs z}}_{\in [0, 1)}\right)^k \xrightarrow[K_1,K_2 \to \pinfty]{} 0.
\]
...
En effet~:
\[\norm {f(z)(z-S)^{-1} - \sum_{k=0}^K\alpha_kz^k(z-S)^{-1}}  \abs {\sum_{k \geq K+1}\alpha_kz^k}\norm {(z-S)^{-1}}.\]
