...
par densité de $D^\infty_0$ dans $L^2$, $\exists (f_k)_{k \in \N} \in {C^\infty_0}^\N \tq f_k \xrightarrow[k \to \pinfty]{L^2} f$. De plus, $(f_k)$ est de Cauchy dans $C^\infty_0$.
...
Donc $\underbrace {x_k+y_k}_{\in F} \xrightarrow[k \to \pinfty]{} x+y$.
...
Montrons également que ce dernier ensemble est fermé dans $L^2$. Soit $f \in \overline E$. Il existe $(f_k)_{k \in \N} \in E^\N \tq f_k \xrightarrow[k \to \pinfty]{L^2} f$.
\[\int_\Omega \abs {u-f}^2\dx = \int_\Omega\abs {f_k-f} \leq \int_{\R^n}\abs {f_k-f}^2\dx \xrightarrow[k \to \pinfty]{} 0.\]
\end{ex}
...
Par convexité de $E$, $(x+y)/2 \in E$, donc $\norm {(x+y)/2} \geq \delta$. En multipliant de part et d'autre par $1/4$ au préalable, on trouve alors~:
\[\norm {x-y}^2 \leq 2(\norm x^2 + \norm y^2) - 4\delta^2 = 0.\]

On en déduit $\norm {x-y} = 0$, i.e. $x=y$.

\exis Soit $(y_k)_{k \in \N} \in E^\N \tq y_k \xrightarrow[k \to \pinfty]{} \delta$ qui existe par définition de l'infimum.
\[\norm {y_k - y_m}^2 \leq 2(\underbrace {\norm {y_k}^2+\norm {y_m}^2)}_{\to 2\delta^2} - 4\delta^2.\]
