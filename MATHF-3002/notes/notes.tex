...
Alors $\forall \xi \in \R^n : \spectre\left(\sum_{j=1}^m\xi_j A_j\right) \subset \R$.
\end{thm}
...
\begin{equation}\label{eq:fin Petrowsky}
	\sup_{\abs t \leq T}\abs {u(t, 0)} \leq C\sum_{\abs \beta \leq p}\sup_{» \in \R^n}\abs {\partial^\beta u^0(x)}.
\end{equation}
...
\begin{ex}~
\begin{enumerate}
	\item Pour $m=n=1$~: $L = \pd {}t + a\pd {}x$. Pour $\xi \in \R : A(\xi) = a\xi$. Donc la condition de Petrowsky revient à imposer que $a \in \R$.
	\item On prend $L = \partial^2_t + 2b\partial^2_{t,x} + c\partial^2_x$ et l'EDP associée $Lu = f$. En posant $v = \partial_t u$ et $w = \partial_x u$, l'EDP devient~:
	...
	\item Pour $\sigma_1 = \begin{bmatrix}0 & 1 \\1 & 0\end{bmatrix}$, $\sigma_2 = \begin{bmatrix}0 & i \\-i & 0\end{bmatrix}$, $\sigma_3 = \begin{bmatrix}1 & 0\\0 & -1\end{bmatrix}$,
...
\begin{proof} Si une solution $u$ existe, alors à $t \in [-T, T]$ fixé, on peut appliquer Fourier en $x$~:
...
Posons~:
\begin{align*}
	P(t, \xi) &\coloneqq e^{-itA(\xi)}\widehat {u^0}
...
$(t, \xi) \mapsto e^{-itA(\xi)}$ est $C^\infty$, donc $(t, \xi) \mapsto e^{-itA(\xi)}\widehat {u^0}(\xi)$ est $C^\infty$ également, i.e. $P \in C^\infty$.
...
\begin{claim}\label{claim 2} Pour $\mu \in \N^n$~: $\exists C > 0, e_\mu > 0 \st \abs {D_\xi^\mu e^{-itA(\xi)}} \leq C(1+\abs\xi)^{e_\mu}$.
\end{claim}
...
on a donc bien finalement $\xi^\alpha\partial_t^kD_\xi^\beta P$ borné. On a exactement la même chose pour $Q$.
\end{proof}
% Crevindiou. P'tite binouze, c'est mérité.

\begin{proof}[\textnormal {\textbf {Démonstration} (de l'Affirmation~\ref{claim 1})}] À venir
\end{proof}

\begin{proof}[\textnormal {\textbf {Démonstration} (de l'Affirmation~\ref{claim 2})}] À venir
\end{proof}
\end{document}
